...
**Após toda modificação de código, siga SEMPRE esta sequência:**

1. **Documentar no README.md** (se necessário): mudanças de comportamento externo, novos endpoints, novas variáveis de ambiente, ou funcionalidades visíveis ao usuário devem ser refletidas no README.md.
2. **Commit** no padrão do projeto (conventional commits) — ver seção abaixo.
3. **Deploy** do serviço afetado — ver seção "Deploy em Produção".

Nunca faça deploy sem commit primeiro. Nunca finalize uma tarefa sem commit.

---

## Deploy em Produção

Toda modificação de código **deve** ser seguida de commit + deploy. Nunca faça deploy sem commit primeiro.

### URLs de Produção
| Serviço | URL |
|---|---|
| Backend (Fly.io) | `https://spy-dash-backend-dark-log-5876.fly.dev` |
| Frontend (Vercel) | `https://spy-dash-frontend.vercel.app` |

### Fluxo de Deploy

```bash
# 1. Commit no padrão do projeto (conventional commits em pt-BR)
git add <arquivos>
git commit -m "feat: descrição curta da mudança"

# 2a. Deploy do backend (se mudou algo em backend/)
cd backend && fly deploy

# 2b. Deploy do frontend (se mudou algo em frontend/)
cd frontend && vercel --prod --yes
```

### Padrão de Commits (Conventional Commits)
Os prefixos seguem o padrão já usado no repositório:
...
### Quando Fazer Deploy de Cada Serviço
- **Backend** (`cd backend && fly deploy`): qualquer mudança em `backend/src/`, `backend/Dockerfile`, `backend/fly.toml`, ou variáveis de ambiente no Fly.io.
- **Frontend** (`cd frontend && vercel --prod --yes`): qualquer mudança em `frontend/src/`, `frontend/.env.production`, ou `frontend/vite.config.ts`.
- **Ambos**: mudanças de tipos compartilhados (ex: `PricePoint`, contratos de API).

### Variáveis de Ambiente de Produção
- **Backend:** gerenciadas via `fly secrets set` — nunca em arquivos commitados.
- **Frontend:** em `frontend/.env.production` — contém apenas variáveis `VITE_*` (públicas por design).
- Para adicionar/atualizar um secret no Fly.io: `fly secrets set CHAVE=valor --app spy-dash-backend-dark-log-5876`
...
### Sistema de Design
Todos os componentes do dashboard devem usar exclusivamente os tokens CSS definidos em `frontend/src/styles/globals.css`. É proibido usar classes Tailwind genéricas de cor (`gray-*`, `blue-*`, `green-[0-9]*`, `white`, `black`) — use sempre os tokens do sistema:

| Token | Valor | Uso |
|---|---|---|
| `bg-card` | `#12121a` | fundo de cards |
| `bg-bg-elevated` | `#1a1a26` | containers principais e superfícies elevadas |
| `border-border-subtle` | rgba(255,255,255,0.06) | bordas padrão |
| `text-text-primary` | `#e8e8f0` | texto principal |
| `text-text-secondary` | `#8888aa` | texto secundário |
| `text-text-muted` | `#555577` | labels, metadados |
| `#00ff88` | accent verde | bullish, ativo, confirmação |
| `#ff4444` | accent vermelho | bearish, erro, alerta |
| `#ffcc00` | accent amarelo | neutro, cautela |
| `#4488ff` | accent azul | info, referência |

O painel **AIPanel** (`frontend/src/components/ai/AIPanel.tsx`) é o padrão de referência visual. Qualquer novo componente deve ter aparência consistente com ele.

### Proibição de Emojis
**É terminantemente proibido o uso de emojis** em qualquer parte do dashboard — painéis, botões, labels, placeholders, mensagens de estado ou qualquer elemento de UI. Isso inclui emojis Unicode como 🛡️, 📊, ⚡, ✅, ⚠️, 🤖, 🔍 e similares. Use texto simples quando necessário; para alertas use `[!]`.
