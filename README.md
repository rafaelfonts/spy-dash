...
- **Prompt base limpo:** dados de mercado essenciais (SPY, VIX, IV Rank, option chain com análise de liquidez bid-ask, GEX, Volume Profile intraday com POC/VAH/VAL, VIX Term Structure, indicadores técnicos, memória de análises); veto block quantitativo com 5 checks + hard vetos (hard veto ativo força `trade_signal: 'avoid'` independente do score)
- **Tool Calling condicional (`fetch_24h_context`):** o modelo decide autonomamente quando buscar o contexto macro (Fear & Greed, VIX term structure, FRED, BLS, earnings, eventos econômicos). Chamado quando VIX > 20, P/C ratio atípico, RSI extremo + crossover MACD, evento macro de alto impacto (FOMC, CPI, NFP, PPI) nas próximas 48h, ou o usuário fizer perguntas macro — evita enviar ~3KB de dados desnecessários em análises de rotina
...
- **GEX (Gamma Exposure):** calculado a partir de option data real da Tradier (`tradierOptionData.ts` + `gexCalculator.ts`)
  ...
  - **Term Structure Scanner (0–60 DTE):** substitui os 5 buckets fixos (0DTE/1D/7D/21D/45D) por varredura dinâmica de toda a cadeia disponível. `selectStructuralExpirations()` em `gexService.ts` mapeia as expirations reais para 8 landmarks institucionais `[0, 3, 7, 14, 21, 35, 45, 60 DTE]`, priorizando OPEX mensais (3ª sexta-feira) em caso de empate, com cap de 8 expirations. `calculateDynamicGex()` computa GEX em paralelo via `Promise.allSettled()` e normaliza `gammaAnomaly = |netGamma| / max(|netGamma|)` (0–1) para identificar onde a concentração de gamma é máxima. Publicado como `gexDynamic` (array `GEXExpirationEntry[]`) no evento SSE `advanced-metrics`; cache por expiração (`gex:exp:{symbol}:{exp}`, TTL 5min)
  - Métricas por expiração: `total`, `callWall`, `putWall`, `flipPoint`, `regime`, `maxGexStrike`, `gammaAnomaly`, `label` (ex: `MAR-21 (14D) OPEX`); formato tabular injetado no prompt IA via `buildGexMultiDTEBlock()` iterativo (framework de 4 camadas: regime vol → GEX term structure → strike PoP → técnico)
...
- Painel `GEXPanel.tsx` no frontend com mini BarChart de term structure (72px, barras verde/vermelho por regime, barra selecionada em destaque); dropdown dinâmico com labels reais (ex: `MAR-21 (14D) OPEX ⚡`) auto-selecionando o entry com maior `gammaAnomaly`; badge de anomalia (Alta ⚡ / Média / Baixa); gráfico de barras por strike (calls verde, puts vermelho); barra do `maxGammaStrike` destacada em laranja via Recharts `Cell`; linhas de referência verticais: preço atual (branco), Flip Point (amarelo), Put Wall (roxo), Call Wall (azul), Max Gamma (ciano), **ZGL** (laranja pontilhado), **VT** (rosa pontilhado); badges VT (verde se SPY > VT, vermelho se SPY < VT, com % distância) e ZGL (distância % do SPY)
...
- **Tabela Supabase:** `portfolio_positions` (id, symbol, strategy_type, open_date, expiration_date, short_strike, long_strike, short_option_symbol, long_option_symbol, credit_received, status OPEN/CLOSED, comments TEXT). Migrations: `20260305000000_portfolio_positions.sql` (schema base) + `20260307000000_portfolio_comments.sql` (campo comments para tese do trade).
- **Serviço:** `portfolioTrackerService.ts` — executa **1x ao dia às 16:00 ET** (scheduler com verificação a cada 60s). Lock Redis `lock:portfolio_tracker:YYYY-MM-DD` (TTL 300s) evita duplicidade em HA. No bootstrap do servidor, `refreshPortfolioSnapshot()` é chamado em paralelo para popular o cache antes de servir a primeira requisição (elimina necessidade de clique em "Atualizar" no cold start).
- **Ciclo:** (1) Busca posições com status OPEN no Supabase; (2) Calcula DTE em **dias corridos** (calendar days) com `diasCorridosEntre(hoje_ET, expiration_date)` — fins de semana incluídos; (3) Tradier `getQuotes()` para short/long (uma chamada batelada); (4) `current_debit = short_ask - long_bid`, `profit_percentage = ((credit_received - current_debit) / credit_received) * 100`, `profit_loss_dollars = (credit_received - current_debit) * 100` (P/L monetário em $); (5) Payload enriquecido enviado ao Claude Sonnet 4.6 (Gestor de Risco).
...
- **Briefing Pós-Fechamento:** `preMarketBriefing.ts` agora injeta seção `### Portfólio Ativo` no prompt do pós-market (16:15 ET), com DTE, lucro %, P/L $ e custo de fechar de cada posição OPEN — ou "Carteira Zerada" se não houver posições abertas.
...
- **Painel no dashboard:** card "Carteira — Put Spreads" (`PortfolioPanel.tsx`) posicionado acima do AIPanel. Tabela com colunas: Estratégia/Tese, DTE, Lucro / P&L $, Crédito, Custo Fechar. Card de lucro didático: % em destaque (verde ≥50%, amarelo 0–49%, vermelho negativo), P/L $ abaixo, barra de progresso visual 0→50% com legenda "Alvo: 50%". Comentários da tese exibidos como subtexto truncado (tooltip no hover). Modal de cadastro (`AddPositionModal.tsx`) com **seletor de estratégia** (Put Spread, Call Spread, Iron Condor), formulário 2 pernas ou 4 pernas, "Gerar símbolos OCC" (P/C), e campo "Comentários (opcional)" para registrar a tese do trade. Hook `usePortfolio.ts` consome os endpoints com auto-fetch ao montar.
...
- **Motor de Gestão de Ciclo de Vida:** scheduler 16:00 ET para Put Spreads (posições OPEN em `portfolio_positions`); DTE em dias corridos + lucro % + P/L monetário ($) via Tradier; Claude Gestor de Risco (FECHAR_LUCRO / FECHAR_TEMPO / ROLAR / MANTER); alertas Discord com embeds (verde 50%, amarelo 21 DTE). Painel **Carteira** no dashboard com snapshot em memória populado no bootstrap, auto-fetch ao montar, campo `comments` para tese do trade, card de lucro didático com barra de progresso e alvo 50%, Cadastrar (modal com gerador OCC + campo comentários), Excluir por linha, endpoints GET/POST portfolio, POST analyze, POST/DELETE positions. Briefing pós-market (16:15 ET) inclui auditoria automática da carteira no prompt da IA.
